...
import { fetchAllUsers, updateUser, deleteUser } from "../../redux/AllUserSlice";
...
const AllCustomer = () => {
  ...
  const handleUpdate = async () => {
    try {
      await dispatch(updateUser({ id: selectedUser.id || selectedUser._id, userData: formData })).unwrap();
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6 text-black overflow-x-auto relative">
      ...
      {/* Toast */}
      {toast.message && (
        <div className={`fixed top-5 right-5 px-4 py-2 rounded shadow text-white ${toast.type === "success" ? "bg-green-600" : "bg-red-600"}`}>
          {toast.message}
        </div>
      )}
          ...
          {users.length > 0 ? users.map(user => (
            <tr key={user.id || user._id}>
              ...
              <td className="px-6 py-4 text-center space-x-2">
                <button onClick={() => openModal(user, "view")} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded">View</button>
                <button onClick={() => openModal(user, "edit")} className="bg-yellow-600 hover:bg-yellow-700 text-white px-3 py-1 rounded">Edit</button>
                <button onClick={() => openModal(user, "delete")} className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">Delete</button>
              </td>
            </tr>
          )) : (
            <tr>
          ...
          <div className="bg-white p-6 rounded shadow-lg w-96 relative">
            ...
            {modalType === "edit" && (
              <div>
                ...
                <input type="text" name="full_name" value={formData.full_name} onChange={handleInputChange} placeholder="Full Name" className="border p-2 w-full mb-2 rounded"/>
                <input type="text" name="username" value={formData.username} onChange={handleInputChange} placeholder="Username" className="border p-2 w-full mb-2 rounded"/>
                <input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="Email" className="border p-2 w-full mb-2 rounded"/>
                <input type="text" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="Phone" className="border p-2 w-full mb-2 rounded"/>
                <input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="Address" className="border p-2 w-full mb-2 rounded"/>
                <select name="role" value={formData.role} onChange={handleInputChange} className="border p-2 w-full mb-2 rounded">
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <button className="bg-green-600 text-white px-3 py-1 rounded" onClick={handleUpdate}>Update</button>
                </div>
              </div>
            )}
            ...
            {modalType === "delete" && (
              <div>
                ...
                <div className="flex justify-end space-x-2 mt-4">
                  <button className="bg-gray-300 px-3 py-1 rounded" onClick={closeModal}>Cancel</button>
                  <button className="bg-red-600 px-3 py-1 text-white rounded" onClick={handleDelete}>Delete</button>
                </div>
              </div>
            )}
          </div>
